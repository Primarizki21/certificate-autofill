...
> Generasi: 2026-08-14 14:18:02 | GT v9 + matcher v2 | baseline = offline_prod (produksi PROD-002, flag ON) | 0 LLM call | gate: >=+2 exact (keputusan user — scope sengaja kecil, +5pt = FAIL by design)

## Hasil

| Metrik | baseline | AKT-004 | delta | gate |
|---|---|---|---|---|
| nama_kegiatan_sertifikasi exact | 6.8% | 54.1% | +47.3% | >=+2 exact |
...
| MACRO exact | 63.0% | 72.1% | +9.1% | no-regress |

## Verdict

**GATE PASS** — nama_kegiatan exact 6.8% → 54.1% (+35 exact, gate >=+2); no-regress: tidak ada.
...
| stem | old (v3) | new (v4) | anchor# |
|---|---|---|---|
| SDC Uniska_Faiz | None | 'Statistic Data Champions' | 19 |

## OOD noise (QA, bukan gate)

- Noise 10%: baseline 5.4% (drop +1.4pt) | AKT-004 48.6% (drop +5.4pt, ekstra +4.1pt)
- Noise 25%: baseline 5.4% (drop +1.4pt) | AKT-004 43.2% (drop +10.8pt, ekstra +9.5pt)

> Drop ekstra vs baseline = harga rule teks-bergantung (sama pola AKT-002/003). Gain absolut tetap positif di semua level noise. Bila di-port produksi: pola KEEP/GUARD.