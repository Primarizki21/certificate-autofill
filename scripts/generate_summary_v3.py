"""
Generate Rangkuman Kurasi Paper Findings V3 — Certificate Autofill
Method Catalog Edition — Pipeline Component Centric + Table of Contents
...
def add_toc(doc):
    ...
    def _make_field_element(para, fld_char_type, instr_text=None):
        run = para.add_run()
    ...
    _make_field_element(para, 'begin')
    _make_field_element(para, None, ' TOC \\o "1-3" \\h \\z \\u ')
    _make_field_element(para, 'separate')
    placeholder = para.add_run('[Daftar Isi: klik kanan → Update Field setelah buka di Word]')
    placeholder.font.size = Pt(10)
    placeholder.font.color.rgb = RGBColor(0x80, 0x80, 0x80)
    placeholder.italic = True
    _make_field_element(para, 'end')
...
def build_document():
    ...
    run = subtitle.add_run('V3 — Method Catalog Edition + Daftar Isi')
    run.font.size = Pt(18)
    ...
    # === SIMPAN ===
    output_path = './docs/rangkuman_findings_v3.docx'
